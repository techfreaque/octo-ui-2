...
export default function JsonEditor(props) {
  const storageName = props.storageName
    ? `$${props.storageName}`
    : "$JsonEditors";
  const HtmlEditorId = `json-editor-${storageName}-${props.editorName}`;
  ...
  function createEditor(props) {
    ...
    const editorElement = document.getElementById(HtmlEditorId) as HTMLElement;
  ...
  useEffect(() => {
    if (props.schema) {
      if (
        ...
        JSON.stringify(window[storageName][props.editorName].schema) ===
...
JsonEditor.propTypes = {
  ...
  show_errors: PropTypes.oneOf(["interaction", "change", "always", "never"]),
