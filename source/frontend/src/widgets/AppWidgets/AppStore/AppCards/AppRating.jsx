...
export default function AppRating({app, rating, votes, style}) {
    ...
    return (<div style={
        ...
        <Tooltip title={
            votes ? `${votes} users have voted ~${rating} stars on average` : `Nobody has rated this ${
            ...
            <span className='ratingComponent__votes'> {votes}</span>
        </Tooltip>
    </div>);
}
...
function Rating({rating, starId, onRatingChange}) {
    ...
    if (rating >= starId + 1) {
        return (<FullStar onRatingChange={onRatingChange}
            starStyle={starStyle}/>)
    } else if (rating >= starId + .5) {
        (<HalfStar onRatingChange={onRatingChange}
            starStyle={starStyle}/>)
    } else {
        return (<EmptyStar onRatingChange={onRatingChange}
            starStyle={starStyle}/>)

    }
}
function FullStar({onRatingChange, starStyle}) {
    return (<svg onClick={onRatingChange}
        style={starStyle}
        width="14"
        height="14"
        viewBox="0 0 20 20"
