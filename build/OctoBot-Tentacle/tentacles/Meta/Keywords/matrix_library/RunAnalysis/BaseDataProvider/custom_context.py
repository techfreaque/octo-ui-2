...
class Context(databases.CacheClient):
    def __init__(
        ...
        self.orders_writer = (
            databases.RunDatabasesProvider.instance().get_orders_db(
                bot_id, self.exchange_name
            )
            if bot_id
            else None
        )
        self.trades_writer = (
            databases.RunDatabasesProvider.instance().get_trades_db(
                bot_id, self.exchange_name
            )
            if bot_id
            else None
        ...
        self.transaction_writer = (
            databases.RunDatabasesProvider.instance().get_transactions_db(
                bot_id, self.exchange_name
            )
            if bot_id
            else None
        ...
        self._sanitized_traded_pair = (
            symbol_util.merge_symbol(self.symbol) if self.symbol else self.symbol
        )
        ...
        self.tentacles_requirements = tentacles_manager_models.TentacleRequirementsTree(
    ...
    def get_signal_builder(self) -> trading_signals.TradingSignalBundleBuilder:
        if self.signal_builder is None:
            self.signal_builder = trading_signals.TradingSignalBundleBuilder(
                self.tentacle.get_trading_signal_identifier(), self.tentacle.get_name()
            )
    ...
    def set_is_trading_signal_emitter(self, is_trading_signal_emitter):
        self.tentacle.trading_config[
            common_constants.CONFIG_EMIT_TRADING_SIGNALS
        ] = is_trading_signal_emitter

    async def emit_signal(self):
        if (
            self.signal_builder is None
            or not self.is_trading_signal_emitter()
            or self.exchange_manager.is_backtesting
        ):
            return
        signal_bundle = self.signal_builder.build()
        self.logger.debug(
            f"Emitting trading signal for {self.signal_builder.strategy}: {signal_bundle}"
        )
        await signals.emit_signal_bundle(signal_bundle)
    ...
    def adapted_trigger_timestamp(self, tentacle_class, config_name):
        ...
        try:
            if (
                isinstance(self.tentacle, modes.AbstractTradingMode)
                ...
                self.trigger_cache_timestamp = self._get_adapted_trigger_timestamp(
                    tentacle_class, previous_trigger_cache_timestamp, config_name
                )
            yield self
        finally:
    ...
    async def local_nested_tentacle_config(
        self, tentacle_class, config_name, is_nested_tentacle
    ):
        ...
        try:
            ...
            self.tentacles_requirements = self.tentacles_requirements.get_requirement(
                tentacle_class, config_name
            )
            yield self
        finally:
    ...
    async def nested_call_context(self, tentacle):
        ...
        context.tentacles_requirements = (
            self.tentacles_requirements
        )  # keep the same tentacles_requirements
        context.parent_tentacles_requirements = self.parent_tentacles_requirements
        context.add_tentacle_requirement_from_nested_context(
            tentacle, context.config_name
        )
        try:
            yield context
        finally:
    ...
    def add_tentacle_requirement_from_nested_context(
        self, tentacle, config_name
        ...
        if self.tentacles_requirements is None:
            self.tentacles_requirements = (
                tentacles_manager_models.TentacleRequirementsTree(tentacle, config_name)
            )
        return (
            self.parent_tentacles_requirements.add_requirement(
                self.tentacles_requirements
            ),
            self.tentacles_requirements,
        )

    def add_referenced_tentacle_requirement(
        self, tentacle_class, config_name
    ) -> (bool, tentacles_manager_models.TentacleRequirementsTree):
        """
        ...
        :return True if the requirement was not already registered and the created requirement
        """
        requirement = tentacles_manager_models.TentacleRequirementsTree(
            None, config_name, tentacle_class=tentacle_class
        )
        return self.tentacles_requirements.add_requirement(requirement), requirement

    def _get_adapted_trigger_timestamp(
        self, tentacle_class, base_trigger_timestamp, config_name
    ):
        try:
            if self.has_cache(
                self.symbol,
                ...
                cache = self.get_cache(
                    tentacle_name=tentacle_class.get_name(), config_name=config_name
                )
                if cache.metadata.get(
                    common_enums.CacheDatabaseColumns.TRIGGERED_AFTER_CANDLES_CLOSE.value,
                    False,
                ):
                    return (
                        base_trigger_timestamp
                        - common_enums.TimeFramesMinutes[
                            common_enums.TimeFrames(self.time_frame)
                        ]
    ...
    async def ensure_tentacle_cache_requirements(self, tentacle_class, config_name):
        ...
        """
        added, requirement = self.add_referenced_tentacle_requirement(
            tentacle_class, config_name
        )
        if added and self._requires_cache_sync_synchronization():
            ...
            registered_requirement = self.get_cache_registered_requirements(
                tentacle_class.get_name(), config_name
            )
            try:
                import octobot_evaluators.evaluators as evaluators

                requirement.tentacle = (
                    evaluators.create_temporary_evaluator_with_local_config(
                        tentacle_class,
                        tentacles_setup_config,
                ...
                self.logger.error(
                    "Octobot-Evaluators is required for ensure_tentacle_cache_requirements"
                )
            await self._reset_cache()
    ...
    def get_config_name_or_default(tentacle_class, config_name):
        return (
            f"nested_{tentacle_class.get_name()}_config"
    ...
    def get_tentacle_config_elements(self, tentacle_class, config_name, config):
        ...
        tentacles_setup_config = (
            self.tentacle.tentacles_setup_config
            if hasattr(self.tentacle, "tentacles_setup_config")
            else self.exchange_manager.tentacles_setup_config
        )
        return (
            config_name,
            cleaned_config_name,
            config,
            tentacles_setup_config,
            self.tentacle.get_local_config(),
        )

    def _requires_cache_sync_synchronization(self):
        registered_requirements = self.get_cache_registered_requirements()
        return (
            registered_requirements is not None
            and not registered_requirements.includes_nested_requirements(
                self.tentacles_requirements
            )
        )
    ...
    async def _reset_cache(self):
        ...
        self.init_cache()
        self.logger.debug(
            f"Replaced cache for {self.tentacle.get_name()} ({self.config_name}) to include "
    ...
    async def get_cached_value(
        ...
        try:
            ...
            if not ignore_requirement:
                await self._try_add_tentacle_as_cache_requirement(
                    tentacle_name, config_name
                )
    ...
    async def get_cached_values(
        ...
        try:
            ...
            return await self.get_cache(
                ...
                max_timestamp, name=value_key, limit=limit, min_timestamp=min_timestamp
            )
    ...
    async def _try_add_tentacle_as_cache_requirement(self, tentacle_name, config_name):
        try:
            if tentacle_name is not None:
                tentacle_class = (
                    tentacles_manager_api.get_tentacle_class_from_string(tentacle_name)
                    if isinstance(tentacle_name, str)
                    else tentacle_name
                )
                await self.ensure_tentacle_cache_requirements(
                    tentacle_class, config_name
                )
        except Exception as e:
            self.logger.exception(
                e, True, f"Error when adding tentacle as a requirement {e}"
            )
    ...
    async def backtesting_results(
        self, with_lock=False, cache_size=None, database_adaptor=databases.TinyDBAdaptor
    ):
        ...
        if not await run_dbs_identifier.exchange_base_identifier_exists(
            self.exchange_name
        ):
        ...
        async with databases.MetaDatabase.database(
            run_dbs_identifier, with_lock=with_lock, cache_size=cache_size
        ) as meta_db:
